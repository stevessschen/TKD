# AICUP 2025 金融異常交易偵測競賽

## 專案概述

本專案參加 AICUP 2025 E.SUN 人工智慧競賽，目標是基於帳戶交易網絡預測哪些帳戶將在未來 30 天（day 122-151）被標記為可疑帳戶。使用 day 1-121 的歷史交易資料，結合圖神經網絡（GCN）與變分自編碼器（VAE），最終在 Private Leaderboard 達成 **F1 Score 0.34**，進入前 30 強。
...
### 4. 特徵選擇 - F1 0.34 (+19.3%) ⭐ 最終突破

**方法**：LightGBM 特徵重要性排序

```python
步驟 1：訓練 LightGBM 分類器（27 維特徵）
步驟 2：計算 Gain-based 特徵重要性
步驟 3：保留前 15 個最重要特徵
步驟 4：重新訓練 GCN-VAE（15 維特徵）
```
