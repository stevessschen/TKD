...
本專案目標是基於帳戶交易網絡預測哪些帳戶將在未來 30 天（day 122-151）被標記為可疑帳戶。使用 day 1-121 的歷史交易資料，結合圖神經網絡（GCN）與變分自編碼器（VAE）模型訓練、驗證與預測，最終在 Private Leaderboard 達成 **F1 Score 0.34**，進入前 20 強。
...
5. ** 執行程式 ** => python main.py (在多個cpu環境下執行約20~30分鐘)
...
- **驗證集表現**：使用 find_best_threshold 尋找最佳閾值 = 0.45
  - Precision: 0.9126
  - Recall: 0.8789
  - F1 Score: 0.8954
- 最終在測試資料中，預測為異常: 470 個帳戶為可疑警示帳戶 (9.83%)
